...
exports.run = async(client, message, args) => {
  ...
  let destekKanal = "626650487629938712";/// buraya canlı destek mesajı atılacak kanal idisi
  ...
  client.channels.get(destekKanal).send(`Destek çağrısına bağlanmak için \`kabulet\`, iptal etmek için \`iptal\` yazınız.`)
  collector.on('message', (message) => {
    if (message.content === 'iptal') collector.stop('aborted')
    if (message.content === 'kabulet') collector.stop('success')
  })
